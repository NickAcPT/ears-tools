...
<script lang="ts">
    import { currentEarsFeatures, emissiveSkin, getEarsFeatures } from "$lib/stores.svelte";
...
<div>
    ...
    <div class="flex h-min gap-2">
        <SkinDropZone onfiles={handleCapeFile} offerDemoSkin={false}>
        ...
        {#if getEarsFeatures().cape}
            <div class="flex flex-1 items-start gap-2">
                ...
                <button class="mt-2" onclick={() => (getEarsFeatures().cape = undefined)}>Remove</button>
            </div>
        {/if}
    </div>
</div>
<div>
    ...
    <input type="range" id="chest-size" min="0" max="1" step="0.1" bind:value={currentEarsFeatures.current.chestSize} />
</div>
                        ...
                        {:else}
                            {#each getEarsFeatures().emissives.palette as pixel, i (i)}
                                <div class="flex h-9 flex-row items-center gap-2">
                                    <input onchange={(e) => handleEmissiveChange(e, i)} type="color" value={rgb_to_hex(pixel)} />
                                    <input
                                        class="w-[8ch]"
                                        onchange={(e) => handleEmissiveChange(e, i)}
                                        type="text"
                                        value={rgb_to_hex(pixel)}
                                    />
                                    <button onclick={() => deleteEmissive(i)}><DeleteIcon class="h-5" /></button>
                                </div>
                            {/each}
                        {/if}
                        {#if getEarsFeatures().emissives.palette === undefined || getEarsFeatures().emissives.palette.length < 16}
                            <button class="h-9" onclick={() => (getEarsFeatures().emissives.palette.push(0))}>Add</button>
                        {/if}
                    </div>
                </div>
            </div>
